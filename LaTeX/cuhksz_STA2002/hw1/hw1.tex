...
\begin{problem}
    ...
    Given observations, i.e. $n_1$ = 200, $X_1$ = 150, $n_2$ = 250, $X_2$ = 185, \\
    $ P_1-P_2 = \frac{X_1}{n_1} - \frac{X_2}{n_2}= \frac{150}{200} - \frac{185}{250} = 0.75 - 0.74 = 0.01$ \\
    As $n_1$ and $n_2$ are sufficiently large (larger than 30), CLT could be applied.\\
    By CLT, the sampling distribution of sample proportions will be approximately normal. \\
    Used as an approximation, $p_1 - p_2 \approx P_1 - P_2 =  0.01$. \\ 
    $\mu(P_1-P_2) = p_1 - p_2 = 0.01$  \\
    Use variance of binomial as an approximation, \\
    Var($X$) = $np(1-p)$, Var($\frac{X}{n}$) = $\frac{p(1-p)}{n}$ \\
    $\sigma(P_1-P_2) = \sigma(P_1) + \sigma(P_2) =  \frac{0.75(1-0.75)}{200} + \frac{0.74(1-0.74)}{250} = 0.001707 $\\
...
\begin{problem}
    ...
    The standard diviation is $\sqrt{\frac{\sigma_1^2}{n_1} + \frac{\sigma_2^2}{n_2}}$.
    \\
    (b)\\
...
\begin{problem}
    \\
    (a)\\
    For an exponential distribution with parametre $\lambda$,\\
    the population mean is $\frac{1}{\lambda}$.\\
    By MoM, $\frac{1}{\lambda} = \bar{x}$.\\
    That is, $\hat{\lambda}_{mom} = \frac{n}{x_1+x_2+\dots+x_n}$.\\
    (b)\\
...
\begin{problem}
    ...
    By MoM, $\frac{\theta + 1}{\theta + 2} = \bar{X}$,\\
...
\begin{problem}
    ...
    The first moment is $E(X) = \frac{a+b}{2}$.\\
